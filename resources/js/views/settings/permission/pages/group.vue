...
export default {
    ...
    methods: {
        ...
        getData(data) {
            ...
            this.service.getGroups(requestData).then((response) => {
        ...
        addModal(data) {
            ...
            this
                ...
                .open({
                    ...
                    contentProps: {
                        title: data!=undefined ? this.$t("permission.group.form.update-title") : this.$t("permission.group.form.add-title"),
                        item: data,
                    },
        ...
        async deleteItem(id) {
            this.$root.showConfirm({
                ...
                content:this.$t('confirm.delete',{a:this.$t('permission.item.name')}),
                accept:()=>{
                     this.service.destroyGroup(id).then((rs) => {                        
...
<template>
<Layout>
    <PageHeader :title="$t(`permission.group.main-title`)" pageTitle="Dashboard" />
    <BRow>
        <BCol cols="12">
            <BCard>
                ...
                <DataTable :value="data" :totalRecords="totalRecords" lazy @page="onPage($event)" @sort="onSort($event)" @filter="onFilter($event)" :pt="{table: 'table',thead: 'table-light'}" paginator :first="fistPageValue" :rows="perPageValue" :rowsPerPageOptions="perPageList">
                    ...
                    <Column :header="$t('permission.group.form.name')" sortable field="name">
                    ...
                    <Column :header="$t('permission.group.form.label')" sortable field="label" style="width: 150px">
                        <template #body="{data}" v-if="!loading">
                            {{ data.label }}
                        </template>
                        <template v-else #body>
                    ...
                    <Column :header="$t('permission.group.permissions')" sortable field="label" style="width: 150px">
                        <template #body="{data}" v-if="!loading">
