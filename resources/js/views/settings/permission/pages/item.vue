...
export default {
    ...
    methods: {
        ...
        getData(data) {
            ...
            this.service.getItems(requestData).then((response) => {
        ...
        addModal(data) {
            ...
            this
                ...
                .open({
                    ...
                    contentProps: {
                        title: data!=undefined ? this.$t("permission.item.form.update-title") : this.$t("permission.item.form.add-title"),
                        item: data,
                    },
...
<template>
<Layout>
    <PageHeader :title="$t(`permission.item.main-title`)" pageTitle="Dashboard" />
        ...
        <BCol cols="12">
            <BCard>
                ...
                <DataTable :value="data" :totalRecords="totalRecords" lazy @page="onPage($event)" @sort="onSort($event)" @filter="onFilter($event)" :pt="{table: 'table',thead: 'table-light'}" paginator :first="fistPageValue" :rows="perPageValue" :rowsPerPageOptions="perPageList">
                    ...
                    <Column :header="$t('permission.item.form.name')" sortable field="name" >
                    ...
                    <Column :header="$t('permission.item.form.label')" sortable field="label" >
                        <template #body="{data}" v-if="!loading">
                            {{ data.label }}
                        </template>
                        <template v-else #body>
